...
- **Order emails**: customers are emailed when payment is confirmed, a prescription is reviewed, the order moves on, staff post an update, or it is cancelled or refunded. Set `RESEND_API_KEY` and `EMAIL_FROM` (a sender on a domain verified in Resend); without them, updates only show on the site. Staff-only notes (e.g. a failed refund) never reach the customer.
- **Privacy policy and terms** (`/privacy`, `/terms`), linked from the footer, sign-up, checkout and the chat. They describe what the code does; have them reviewed before launch, and update them if the services used change.
...
- Chat triage lives in `src/lib/chat/`: `triage.ts` has the emergency phrases (checked before the model, so they never depend on it) and the fixed handover messages; `assistant.ts` has the prompt and tools; `conversation.ts` handles storage, rate limits and handover. The model never writes what the customer is told when a chat is handed over. Health advice is checked against a short list of trusted sites (`TRUSTED_SITES` in `assistant.ts`) and each cited page is shown under the reply; `guard.ts` holds back any reply that states a dose or names a prescription-only medicine.
- Pack labels (`/admin/labels`): directions and warnings from each product's pack. Drafts are staff-only; once a pharmacist approves one it shows on the product page and the chat assistant may quote it word for word (any other dose in a reply is held back). Editing an approved label returns it to draft. `prisma/seed-labels.ts` holds draft labels for the sample catalogue, taken from official leaflets, each with its source link.
- `npm run eval:chat` runs the assistant against the test conversations in `evals/chat/cases.ts` with the real model and writes a report to `evals/chat/results/` for pharmacist review. It costs money each run (printed at the end); run it after any change to the prompt, tools or model. `-- --group escalate` or `-- --only <id>,<id>` runs a subset.
- Security: sign-in (customer and staff), sign-up, password reset, the contact form, order tracking, delivery quotes, checkout and the chat are rate-limited per IP address (and per account for sign-in and password reset), counted in the `RateLimitHit` table so limits hold across Vercel instances; see `src/lib/rate-limit.ts`. `ASSISTANT_DAILY_LIMIT` caps assistant replies per day. Uploads are checked by their contents, not the name or type the browser sends. Session cookies are httpOnly, and `next.config.ts` sets security headers.
